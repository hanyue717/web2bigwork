	var gong=document.getElementById('rightt2');
	...
	var banner = document.getElementById('main2');
	...
	var isMoving = false;
		...
		num1.setAttribute("onclick", "chong()");
		function chong(){
			pay.innerHTML="¥"+num1.value;
		}
...
function animate(obj,json,callback){
    ...
    function next(){
        ...
        index++;
        navmove();
        animate(banner,{left:-800*index},function(){
            if(index==7){
    ...
    timer = setInterval(next, 2000);
		...
		var tofly = document.getElementsByClassName('rightfly')[0].children[0].children;
		var ewei = document.getElementsByClassName("ewei")[0];
		for(var erwei = 0; erwei<4; erwei++){
			(function (erwei){
				tofly[erwei].onmouseover = function (){
					buffer(tofly[erwei],{right:-20});
					if(erwei==2){
						ewei.src = 'img/erwei.png';
						ewei.style.marginTop="0px";
					}
				}
				tofly[erwei].onmouseout = function (){
					buffer(tofly[erwei],{right:-95});
					if(erwei==2){
